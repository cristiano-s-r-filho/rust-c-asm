...
fn main() {
    ...
    // Generate Enviroment: 
    let mut cpu = CPU::new();
    cpu.initialize_cpu_state(&mut process, (0,0,0,0));
    describe_cpu_state(&cpu);
    // let mmu = cpu.crom.mmu;
    ...
    loop {
        ...
        cpu.offsets.write_to_register("eip", inst_adrr);
        cpu.execute_instruction(opcode, num_operands[0], num_operands[1]);
        ...
        if cpu.flag.ovfw == true || keep_going == false {
            break; 
        }
    }
    // END of LOOP. 
